...
enum TestAssets {
    static var projectItem: ProjectItem {
        ProjectItem(title: expectedTitle, content: expectedContent, deadlineDate: deadlineDate, progress: expectedProgress, index: 0)
    }
    static var projectItemWithLongText: ProjectItem {
        ProjectItem(title: "내 본문은 길다구~", content: longText, deadlineDate: deadlineDate, progress: .todo, index: 2)
    }
    static let expectedTitle = "한 번만에 돼라"
    static let expectedContent = "123"
    static let expectedProgress = ProjectItem.Progress.todo
    static let deadlineDate = Date()
    static let expectedDeadlineDate = deadlineDate.formatted(as: "yyyy-MM-dd'T'HH:mm:ssZ")
